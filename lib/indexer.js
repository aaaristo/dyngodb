...
console.log('finish');
                dyn.stream(table._dynamo.TableName)
                   .scan({ limit: 25 }) 
                   .on('data',function (items) { console.log('data',items.length); })
                   .on('end',_.partial(console.log,'eeeend'))
                   .pipe(index.streamElements())
                   .on('data',function (items) { console.log('trans',items.length); })
                   .on('end',_.partial(console.log,'teeeend'))
                   .pipe(mput)
                   .on('error',p.trigger.error)
                   .on('finish',p.trigger.success);
           });
