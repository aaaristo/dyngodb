module.exports= function (dyn)
{
    ...
    finder.find= function (query)
    {
       ...
       var p= dyn.promise(['results','count'],'notfound'),
       ...
       if (query.limit==1)
         dyn.table(query.table.name)
            ...
            .query(query.count ? p.trigger.count : p.trigger.results,
             { attrs: query.projection.include, count: query.count })
            .error(_triggerError);
       else
         dyn.table(query.table.name)
            .hash('$id',query.cond.$id)
            .query(query.count ? p.trigger.count : p.trigger.results,
             { attrs: query.projection.include, limit: query.limit, count: query.count })
            .error(_triggerError);
