...
module.exports= function (dyn,table,fields)
{
     ...
     {
        var elem= { id: item.$id.replace(/-/g,'_')+'__'+item.$pos, version: item.$version, fields: {} };
        elem.lang= item.$lang || domain.lang;

        _.keys(item).forEach(function (field)
        {
            if (field.indexOf('$')!=0)
              elem.fields[field.toLowerCase()]= item[field];
        });
          ...
          else
          {
            ...
            if (query.count&&query.counted)
                ...
                { 
                    ...
                    return { $id: id[0].replace(/_/g,'-'), $pos: parseInt(id[1]) }; 
                })); 
          }
       });        
