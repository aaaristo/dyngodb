...
const  _compare= function (x, y)
       ...
       _operators= {
                      ...
                      {
                         return itemVal.indexOf(vals[0])==0;
                      }),

                      CONTAINS: _operator(function (itemVal,vals)
                      {
                         return itemVal.indexOf(vals[0])>-1;
                      }),
...
module.exports= function (dyn, query)
{
    ...
    {
            if (!((query.orderby&&!query.sorted)||Object.keys(query.$filter).length))
            ...
            function (err)
            {
               if (err)
                 ...
                 trigger(_limit(_sort(_filter(items,query),query),query));
            });
    });
