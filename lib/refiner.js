...
const  _compare= function (x, y)
       ...
       _limit= function (items,query)
       {
            if (!query.limited)
            {
                ...
                {
                    console.log('limiting results in memory'.red);
                    items= items.slice(0,query.limit);
                }

                if (query.skip)
                {
                    console.log('skipping results in memory'.red);
       ...
       _refineItems= function (trigger,items,query)
       {
                ...
                function (err)
                {
                   ...
                   { 
                     items= _modifiers(items,query);

                     if (query.count)
                     {
                       console.log('counting results in memory'.red);
                       refiner.trigger.count(items.length);
                     }
                     else
                       trigger(items);
                   }
...
module.exports= function (dyn, query)
{
    ...
    var refiner= dyn.promise(promises), _end, _results, _items;
