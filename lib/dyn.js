    ...
    dyn.stream= function (table)
    {
        ...
        methods.scan= function (opts)
        {
             var rstream= new Stream(),
                 emit= { 
                         data: _.bind(rstream.emit,rstream,'data'),
                         error:  _.bind(rstream.emit,rstream,'error'),
                         end:  _.bind(rstream.emit,rstream,'end')
                       },
                 _resume,
                 _data= function (items)
                 {
                      emit.data(items);

                      if (!rstream.paused&&items.next)
                        items.next();
                      else
             ...
             rstream.pipe= function (wstream)
             {
                ...
                rstream.on('data',function (items)
                {
