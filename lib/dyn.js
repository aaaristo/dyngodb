...
module.exports= function (opts)
{
    ...
    var _dyn = new AWS.DynamoDB(), 
         ...
         {     
               ...
               {
                  success.forEach(function (type) { var trg= p.trigger[type];  if (trg) promise[type](p.trigger[type]); });
       ...
       process.nextTick(function ()
       {
           ...
           _catch(function (err,data)
           {
                  if (err)
                    ...
                    cb(opts.count ? data.Count : _.collect(data.Items,_item));
           }));
       });
...
//console.log(JSON.stringify(query,null,2));
    ...
    dyn.updateItem= function (opts,cb)
    {
       ...
       _.keys(opts.update).forEach(function (attr)
       {
           ...
           query.AttributeUpdates[attr]= { Action: upd.action, Value: _value(upd.value) };
       });

       var promise= _promise([],'notfound');
...
console.log(UnprocessedItems);
    ...
    //   console.log(JSON.stringify(query,null,2));
       ...
       var promise= _promise(['progress','end']);

       process.nextTick(function scanner(count)
       {
           _dyn.scan(query,
           function (err,data)
           {
//console.log(err,data);
    ...
    dyn.iscli= function ()
    {
       ...
       return a1.indexOf('dyngodb', a1.length - 7) !== -1;
    }
    ...
    dyn.stream= function (table)
    {
        ...
        methods.readable= function (read)
        {
             ...
             {
                wstream.on('drain',function () { console.log('drain'); rstream.resume(); });

                rstream
                    .on('data',function (items)
                    {
    console.log('data pipe',items.length);
                      if (!wstream.write(items))
                        rstream.pause();
                    })
                    .on('end',function (items) { console.log('end pipe'); wstream.end(items); });
        ...
        methods.writable= function (write)
        {
             ...
             {
                 console.log('write',items.length);
        ...
        {
             var rstream= methods.readable(_.identity),
                 wstream= methods.writable(function (items,emit,end)
                          {
                                rstream.emit(end ? 'end' : 'data',transform(items)); 

                                if (end)
                                  emit.done();
                                else
                                  rstream.resume= emit.done;
                          });

             wstream.pipe= _.partial(rstream.pipe);
        ...
        methods.mput= function (op)
        {
             ...
             {
                   var _chunk= function (array,chunk)
                   {
                      var i,j,chunks= [];
                   ...
                   async.forEach(_chunk(items,25),
                   function (items,done)
                   { 
                       dyn.table(table)
                          .mput(_ops(items),done)
                          .error(done);
                   },
                   function (err)
                   {
