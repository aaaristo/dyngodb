...
module.exports= function (opts)
{
    ...
    var _dyn = new AWS.DynamoDB(), 
         ...
         _promise= function ()
         {
               var args= Array.prototype.slice.call(arguments),
                   promise= { queue: {}, trigger: {} };

               _.union(args,['error']).forEach(function (arg)
               {
                   promise.queue[arg]= [];
       ...
       process.nextTick(function ()
       {
           ...
           function (err,data)
           {
                  ...
                  if (!data.Item)
                  {
                    ...
                    promise.trigger.error(_error({ message: 'item not found',
                                                      code: 'notfound'}));
                  }
                  else
    ...
    dyn.put= function (obj,cb,opts)
    {
       ...
       var promise= _promise('notfound','found');
               ...
               function (err,data)
               {
                 if (err)
                 {
                    ...
                    if (err.code=='ConditionalCheckFailedException')
                    {
                        ...
                        {
                          promise.trigger.notfound();
                          promise.trigger.error(_error({ message: 'Object does not exists', code: 'notfound' }));
                        }
                        else
                        {
                          promise.trigger.found();
                          promise.trigger.error(_error({ message: 'Object exists', code: 'found' }));
                        }
                    }
                    else
                      promise.trigger.error(err);
